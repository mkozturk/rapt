...
class Fieldline:
    def __init__(self, tpos, field, ds=0, stopcond=None, Bmin=None, Bmax=None):
        ...
        if ds == 0:
            ...
            self.ds = ds
        if stopcond == None:
            self.stopcond = lambda b,c: False
        else:
    ...
    def trace(self):        
        ...
        while not self.stopcond(forward[-1,:], self.field):
            ...
            if self.solver == 'rkf':
                try:
                    s, r = rkf(derivfw, 0, self.ds,  forward[-1,:], tol = 1e-3, hmax=self.ds, hmin=1e-6)
                except RuntimeError:
                    break
        ...
        while not self.stopcond(backward[-1,:], self.field):
            # Follow the field line for length -ds, starting from the last point.             
            if self.solver=='odeint':
                r = odeint(derivbw, backward[-1,:], [0, self.ds], rtol=1e-6, atol=1e-6)
            if self.solver=='rkf':
                try:
                    s, r = rkf(derivbw, 0, self.ds,  backward[-1,:], tol = 1e-4, hmax=self.ds, hmin=1e-6) 
                except RuntimeError:
                    break
            # Append the new data to existing curve data.
