...
class GuidingCenter:
    ...
    def advance(self, delta, eom="TaoChanBrizardEOM"):
        ...
        Neither Northrop-Teller (as implemented here), nor the Chan-Brizard
...
#    def TaoChanBrizardAdvance_old(self, delta): # to be removed
...
#
#    def advance_test(self, delta): # to be removed
#        # To be used when electric fields are nonzero and/or fields vary in time.
#        # Reference: Tao, X., A. A. Chan, and A. J. Brizard (2007),
#        # Hamiltonian theory of adiabatic motion of relativistic charged particles,
#        # Phys. Plasmas, 14, 092107, doi:10.1063/1.2773702
...
#            Bmag = np.sqrt(np.dot(B,B))
#            unitb = B / Bmag
#            gamma = np.sqrt(1 + 2*self.mu*Bmag/(m*c*c) + (ppar/(m*c))**2)
#            cb = self.field.curlb(tpos)
#            Bstar = B + ppar * cb / self.charge
#            Bstarpar = np.dot(Bstar,unitb)
#            E = self.field.E(tpos)
...
#            
#            retval = np.ones(4) 
#            retval[:3] = (ppar * Bstar / (gamma*m) + np.cross(Estar,unitb) ) / Bstarpar
#            retval[3] = self.charge*np.dot(Estar,Bstar) / Bstarpar
#            return retval
#        if delta <= dt:
#            times = np.array([self.tcur, self.tcur+delta])
...
#        rtol, atol = params["solvertolerances"]
#        
#        traj,infodict = odeint(deriv, self.trajectory[-1,1:], times,
#                               rtol=rtol, atol=atol,full_output=True)
#        traj = traj[1:,:]
#        tarr = infodict['tcur']
#        tarr = tarr.reshape((tarr.shape[0],1))
#        temptraj = np.hstack((tarr,traj))
#        
#        self.trajectory = np.concatenate((self.trajectory, temptraj))
#        self.tcur = self.trajectory[-1,0]
...
#        r.set_initial_value(self.trajectory[-1,1:], self.trajectory[-1,0])
#        while r.successful() and r.t < self.tcur+delta:
#            r.integrate(r.t+dt)            
#            next = np.hstack(([r.t],r.y))
